#!/usr/bin/env bash
# Serve BarkleyScope_folger.py -- Anais's version of the map app, with the Folger
# SST comparison figure as a second tab on the two Folger site panels -- in marimo's
# *app* mode (no code cells, no
...
#
# Usage:  ./serve_folger_app.sh [PORT]     (default 2719)
#
# 2719, not 2718, so this can run alongside the team's app on 2718.
set -euo pipefail

PORT="${1:-2719}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP="$HERE/BarkleyScope_folger.py"
