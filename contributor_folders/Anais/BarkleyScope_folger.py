...
def config():
    CONFIG_MAP = {
        ...
        "GLIDER": {
            # INERT. Both keys below are ignored: `glider_data` reads
            # glider_frozen.parquet from this folder and never calls
            # load_active_gliders() at all. Kept only so the diff against the team's app
            # stays readable, and as a record of what was tried. Changing them does
            # nothing -- to change the data, rebuild the frozen file (see README).
            #
            # The history, since it explains why the freeze exists at all:
            #
            # "live" reads data/cproof_https.py's snapshot() straight from C-PROOF's
            # server on every cold start. That is right for the team's app -- it is a
            # what-is-in-the-water-now panel -- but wrong for this one, which exists to
            # show a fixed figure and should render the same thing every time it is
            # opened, with no network dependency and nothing that can change under a
            # demo. "realtime" reads data/cproof_glider_realtime.nc instead: 3.1 MB,
            # committed to git, no network.
            #
            # The cost is staleness, and it is real: that archive is a daily-refreshed
            # snapshot, and whatever is committed is what you get. Switch back to "live"
            # if you ever want the current picture.
            "MODE": "realtime",
            # 14, not the team app's 1, and the two settings go together.
            #
            # ACTIVE_DAYS is "has a fix in the last N days". Against "live" (current to
            # within the hour) 1 day is a true "active now" filter. Against the committed
            # archive it is not: measured at the time of writing, that file's newest
            # observation was 5 days old, and MAX_HOLD_HOURS additionally masks a glider
            # that sat on one position for 45 h -- so 1, 3 and even 7 days all returned
            # ZERO deployments and the live view rendered empty. 14 returns 1.
            #
            # If the live view comes up empty, this number is the first thing to raise;
            # the archive has simply gone staler than 14 days.
            "ACTIVE_DAYS": 14,
        ...
        "HISTORICAL": {
            ...
            # The Folger comparison figure: satellite skin SST against both ONC depths,
            # four panels, built by data/sst_folger_compare/make_comparison.py.
            #
            # ONE fixed figure rather than a per-site property like `climatology_png`,
            # because it is not about one site: panel 1 overlays the satellite cell that
            # covers BOTH instruments with each station's own record. So Folger Deep and
            # Folger Pinnacle answer a click with the same figure, on purpose.
            "COMPARE_FIGURE": "data/sst_folger_compare/sst_vs_folger_four_panel.png",
            # The layer that figure was drawn from. Read only for its caveat sentences --
            # the numbers in them ("satellite 2019-2026", "5.6 x 3.7 km") are regenerated
            # by export_compare_points.py, so quoting the file beats retyping them here
            # and letting them rot. Same reason `about_note` interpolates `sst_meta`.
            "COMPARE_META": "data/folger_compare_points.geojson",
...
def about_note(mo, sst_meta):
    ...
    **Real-time** -- C-PROOF gliders, read from a **frozen snapshot taken 2026-08-28**
    (`glider_frozen.parquet`, in this app's own folder), **not** live from C-PROOF. This copy
    of the app is deliberately static: it makes no network request of any kind, and shows the
    same three deployments every time it is opened. The team's app at
    `final_notebooks/` still reads the live feed. One point per observation.
...
@app.cell(hide_code=True)
def glider_data(CONFIG_MAP, pd):
    # FROZEN. The team app calls load_active_gliders(), which fetches from C-PROOF's
    # server on every cold start. This copy reads a snapshot taken once, on 2026-08-28,
    # and never touches the network.
    #
    # Why a frozen file rather than MODE="realtime": that mode reads the committed
    # archive data/cproof_glider_realtime.nc, whose newest observation was 5 days old --
    # so it showed ONE stale deployment where the live feed had three current ones. The
    # freeze keeps what the live feed actually had at the moment it was taken.
    #
    # Built by the same code path the team app uses -- load_active_gliders(mode="live",
    # active_days=14) -- so the QC screen, the box clip and the held-position mask have
    # all already been applied. The columns are exactly what that function returns, plus
    # `deployment` and `glider` flattened in so the grouping below can rebuild the same
    # list-of-dicts shape the rest of this app expects.
    #
    # To refresh it (a network fetch), re-run the snippet in this folder's README.
    from pathlib import Path as _P

    _glider_cfg = CONFIG_MAP["GLIDER"]
    glider_var = _glider_cfg["VARIABLE"]
    _frozen = _P(__file__).resolve().parent / "glider_frozen.parquet"

    glider_records = []
    if _frozen.exists():
        _raw = pd.read_parquet(_frozen)
        for _dep, _grp in _raw.groupby("deployment", sort=False):
            _df = _grp.drop(columns=["deployment", "glider"]).reset_index(drop=True)
            if len(_df) >= _glider_cfg["MIN_TRACK_POINTS"]:
                glider_records.append({
                    "deployment": _dep,
                    "glider": _grp["glider"].iloc[0],
                    "df": _df,
                })
    ...
    if not glider_records:
        # Never silent: an empty live view with no explanation reads as a bug in the map.
        print(f"Glider: NO frozen data at {_frozen} -- the real-time view will be empty.")
    else:
        _newest = max(pd.to_datetime(_r["df"]["Time"]).max() for _r in glider_records)
        print(f"Glider: {len(glider_records)} deployment(s) from a FROZEN snapshot "
              f"(no network); newest observation {_newest:%Y-%m-%d %H:%M} UTC")
...
def historical_data(CONFIG_MAP):
    ...
    _here = _Path(__file__).resolve().parents[2]   # repo root: this file is 2 deep
...
@app.cell(hide_code=True)
def folger_compare_data(CONFIG_MAP):
    # The Folger comparison figure and the caveat sentences that go with it.
    #
    # A cell of its own rather than an addition to `historical_data`, for the same
    # reason `sst_data` is its own cell: `map` depends on `historical_data`, so anything
    # added there widens what can make `map` re-run -- and `map` must never re-run (see
    # its own comments). Nothing references this cell except `site_panel`.
    # Private aliases: marimo requires top-level names to be unique across cells, and
    # `json` / `base64` are already imported by other cells.
    import json as _json
    from pathlib import Path as _Path

    _repo = _Path(__file__).resolve().parents[2]

    # Resolved, NOT read. Encoding 461 kB of PNG here would put it on the startup path
    # and defeat the whole point of loading it lazily when the tab is opened -- see
    # `site_panel`. All this cell decides is whether the file is there at all.
    _fig = _repo / CONFIG_MAP["HISTORICAL"]["COMPARE_FIGURE"]
    folger_compare_figure = _fig if _fig.exists() else None

    # Only the collection-level `properties` -- the caveat sentences live there, not on
    # the feature. The 98 kB parse costs ~3 ms, once, against the 1.3 MB of glider track
    # `historical_data` already parses; and quoting the file keeps the numbers in those
    # sentences from drifting out of step with the figure.
    try:
        _meta = _json.loads((_repo / CONFIG_MAP["HISTORICAL"]["COMPARE_META"]).read_text())
        folger_compare_meta = _meta.get("properties") or {}
    except (OSError, ValueError):
        # Same degradation rule as `historical_data`'s `_load`: a broken or absent file
        # costs the caveats, not the figure, and never takes the app down.
        folger_compare_meta = {}

    print(f"Folger compare: figure {'found' if folger_compare_figure else 'MISSING'}, "
          f"{len(folger_compare_meta)} metadata field(s)")
    return folger_compare_figure, folger_compare_meta
...
def sst_data():
    ...
    _sst_path = Path(__file__).resolve().parents[2] / "data" / "sst_barkley_layer.geojson"
...
@app.cell(hide_code=True)
def site_panel(folger_compare_figure, folger_compare_meta, mo, selected_site):
    # The plot(s) for a clicked site, as data URIs.
    #
    ...
    #
    # The two Folger sites additionally get the SST comparison figure, as a second tab.
    # See the `group == "folger"` branch at the bottom for why it is a tab and not a
    # replacement, and why the second tab is lazy.
    import base64
    import html
    from pathlib import Path as _Path
    ...
    if selected_site is None:
        site_plot = None
        site_panel_width = None
    else:
        _repo = _Path(__file__).resolve().parents[2]
        # NOTE two different anchors in this cell, deliberately. `climatology_png` is
        # repo-root-relative because it comes out of climatology_sites.geojson, whereas
        # `folger_compare_figure` was already resolved to an absolute path in
        # `folger_compare_data`. Do not "tidy" one into the other.
        ...
        <style>
          ...
          /* The comparison figure needs more words than the climatology does -- three
             series on one axis invites a reading the data does not support. */
          .clim-help.clim-help-wide .clim-help-body { width: 300px; }
          .clim-help:hover .clim-help-body,
          .clim-help:focus-within .clim-help-body { display: block; }
        </style>
        """

        def _figure(uri, alt, help_html, wide=False):
            # One shape for both tabs: image, with a "?" pinned to its top-right.
            _cls = "clim-help clim-help-wide" if wide else "clim-help"
            return (
                '<div class="clim-figure">'
                f'<img src="{uri}" alt="{alt}" />'
                f'<span class="{_cls}" tabindex="0" role="note" '
                'aria-label="About this plot">?'
                f'<span class="clim-help-body">{help_html}</span></span>'
                "</div>"
            )
        ...
        if _path is not None and _path.exists():
            _clim_body = mo.Html(_figure(
                _data_uri(_path),
                f"Day-of-year temperature climatology for {selected_site['name']}",
                _help,
            ))
        else:
            _clim_body = mo.md(
        ...
        if selected_site.get("group") == "folger" and folger_compare_figure is not None:
            # Both Folger markers answer with the SAME comparison figure: it is about the
            # pair plus the satellite cell covering both, not about one depth. They sit
            # 0.006 deg apart, well inside click_plot's 0.03 deg tolerance, so which one
            # nearest-wins picks does not matter here.
            #
            # A TAB rather than a replacement. Dwight's climatology and this answer
            # different questions about the same place, and dropping his plot from these
            # two sites to make room would be taking something away to add something.
            #
            # Caveats quoted from the layer, not retyped: the numbers in them are
            # regenerated by export_compare_points.py. Two sentences, not all five --
            # these are the ones that change how you READ the picture, and a popover long
            # enough to scroll is one nobody reads (the same rule `about_note` follows).
            _caveats = " ".join(
                html.escape(folger_compare_meta[_k])
                for _k in ("depth_caveat", "baseline_caveat")
                if folger_compare_meta.get(_k)
            )
            _compare_help = (_caveats or
                "Satellite skin SST against two ONC sensor depths at Folger Passage."
            ) + (" Full limitations: <code>data/sst_folger_compare/README.md</code>.")

            def _build_compare():
                # Zero-arg CALLABLE, which is the whole point. mo.ui.tabs renders every
                # tab's HTML at construction (it concatenates them into one slotted_html
                # string), so handing it a built figure would base64 the 461 kB PNG on
                # EVERY Folger click whether or not this tab is ever opened -- ~900 kB a
                # click against ~285 kB today. mo.lazy defers the read and the encode
                # until the tab is actually selected. Note mo.ui.tabs(lazy=True) does NOT
                # do this: it wraps already-constructed objects, so the encode is paid.
                return mo.Html(_figure(
                    _data_uri(folger_compare_figure),
                    "Folger Passage temperature: satellite skin SST against 25 m and 98 m, "
                    "four panels -- daily series, then monthly anomalies per series",
                    _compare_help,
                    wide=True,
                ))

            # Private name, and nothing anywhere reads _tabs.value. marimo drops
            # single-underscore names when working out which cells refer to a UI element,
            # so a private tabs element has no referring cells and a tab click re-runs
            # NOTHING. A public name would silently re-encode both PNGs on every click.
            # (Reading .value in this cell would also raise: a cell may not read the value
            # of an element it defines.) No value= either -- a fresh element per run
            # already resets to the first tab, which is the wanted behaviour when you
            # click a different site.
            _tabs = mo.ui.tabs({
                "Climatology": _clim_body,
                "vs. satellite": mo.lazy(_build_compare, show_loading_indicator=True),
            })
            _panel = _tabs
            # Narrower than the climatology's 600px because this figure is PORTRAIT
            # (2868x3207) where Dwight's are landscape (1650x1125). The panel width is
            # the plot size, so widening this one makes it taller, not bigger.
            site_panel_width = "500px"
        else:
            # Six of eight sites, and Folger when the figure is missing from the
            # checkout: byte-for-byte the panel this app has always shown. The missing
            # case is already reported in the startup console by folger_compare_data.
            _panel = _clim_body
            site_panel_width = "600px"

        # The <style> is a SIBLING of the tabs, never inside one. marimo's tabs unmount
        # the inactive tab's content entirely (Radix TabsContent, no forceMount), so a
        # <style> living in tab 1 vanishes the moment you switch to tab 2 and the second
        # figure renders unstyled -- full-bleed, with the "?" body expanded as inline
        # text. Hoisting it here keeps it mounted for as long as the panel is.
        site_plot = mo.vstack([_header, mo.Html(_style), _panel])
    return site_panel_width, site_plot
...
def plot_overlay(
    ...
    # The site panel decides its own width; this cell only applies it. The PNG is a
    # fixed figure scaled to the panel width, so the panel width IS the plot size -- and
    # which width is right depends on the figure's aspect, which only `site_panel` knows:
    # 600px for Dwight's landscape climatologies (480 left the day-of-year axis cramped),
    # 500px for the portrait Folger comparison, where a wider dock would only make a
    # taller figure. The glider curtain is a plotly figure that reflows, and it sits
    # above two sliders whose labels wrap badly when the dock gets wide, so it keeps the
    # narrowest one.
    _width = site_panel_width or "480px"
